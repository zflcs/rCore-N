...
lazy_static! {
    pub static ref SHARED_SCHE: Arc<Vec<u8>> =
        Arc::new(get_app_data_by_name("sharedscheduler").unwrap().to_vec());
    pub static ref SHARED_SCHE_MEMORYSET: MemorySet =
        MemorySet::from_module(SHARED_SCHE.as_slice());
...
fn add_lkm_image() {
    KERNEL_SPACE
        .lock()
        .add_kernel_module(&SHARED_SCHE_MEMORYSET);
