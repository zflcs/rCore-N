...
impl NetDevice {
    pub fn transmit(&self, data: &[u8]) {
        ...
        net.lock()
            .send(device::net::TxBuffer::from(data))
...
fn get_net_device() -> &'static mut Mutex<VirtIONet<VirtioHal, MmioTransport, NET_QUEUE_SIZE>> {
    unsafe { &mut *(NET_DEVICE as *mut Mutex<VirtIONet<VirtioHal, MmioTransport, NET_QUEUE_SIZE>>) }
}
...
pub fn init() {
    unsafe {
        ...
        let virtio =
            VirtIONet::<VirtioHal, MmioTransport, NET_QUEUE_SIZE>::new(transport, NET_BUFFER_LEN)
                .expect("can't create net device by virtio");
        let net = Arc::new(Mutex::new(virtio));
        NET_DEVICE = net.as_ref()
            as *const Mutex<VirtIONet<VirtioHal, MmioTransport, NET_QUEUE_SIZE>>
            as usize;
        core::mem::forget(net);
    }
}
